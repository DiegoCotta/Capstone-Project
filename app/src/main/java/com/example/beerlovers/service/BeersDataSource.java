...
import com.example.beerlovers.service.Service;
...
public class BeersDataSource extends PageKeyedDataSource<Integer, Beer> {
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull final LoadInitialCallback<Integer, Beer> callback) {
        ...
        if (search == null || search.isEmpty())
            Service.retrofit.create(Service.class).getBeers(BuildConfig.API_KEY, 1).enqueue(new Callback<BaseResponse<Beer>>() {
                @Override
            ...
            Service.retrofit.create(Service.class).searchBeer("beer", search, BuildConfig.API_KEY, 0).enqueue(new Callback<BaseResponse<Beer>>() {
                @Override
                public void onResponse(Call<BaseResponse<Beer>> call, Response<BaseResponse<Beer>> response) {
                    if (response.isSuccessful() && (response.body() != null)) {
                        if (response.body().getData() != null) {
    ...
    public void loadAfter(@NonNull final LoadParams<Integer> params, @NonNull final LoadCallback<Integer, Beer> callback) {
        if (search == null ||
                search.isEmpty())
            Service.retrofit.create(Service.class).getBeers(BuildConfig.API_KEY, params.key).enqueue(new Callback<BaseResponse<Beer>>() {
                @Override
                public void onResponse(Call<BaseResponse<Beer>> call, Response<BaseResponse<Beer>> response) {
            ...
            Service.retrofit.create(Service.class).searchBeer("beer", search, BuildConfig.API_KEY, params.key).enqueue(new Callback<BaseResponse<Beer>>() {
                @Override
                public void onResponse(Call<BaseResponse<Beer>> call, Response<BaseResponse<Beer>> response) {
                    if (response.isSuccessful()) {
                        Integer nextKey = (params.key == response.body().getTotalResults()) ? null : params.key + 1;
